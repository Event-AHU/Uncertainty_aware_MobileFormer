...
class MyDyRelu(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # pdb.set_trace()
        # BxCxHxW -> HxWxBxCx1
        # BxCXTxHxW -> TxHxWxBxCx1
        pdb.set_trace()
        x_perm = x.permute(2, 3, 4, 0, 1).unsqueeze(-1)
        # h w b c 1 -> h w b c k
        # t h w b c 1 -> t h w b c k
        # t h w b c 1 * b c 2 -> t h w b c 2
        output = x_perm * relu_coefs[:, :, :self.k] + relu_coefs[:, :, self.k:]
        # TxHxWxBxCxk -> BxCxTxHxW
        result = torch.max(output, dim=-1)[0].permute(3, 4, 0, 1 ,2)
        return result
