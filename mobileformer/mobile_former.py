...
from mobileformer import torchvision_resnet
from mobileformer.ResNets_3D.models import resnet

# from mobileformer.resnet3d import ResNet3d
import pdb
__all__ = ['MobileFormer']
  
def _cfg(url='', **kwargs):
    return {
        'url': url, 'num_classes': 101, 'input_size': (3, 224,224), 'pool_size': (1,1, 1),
...
class MobileFormer(nn.Module):
    def __init__(
        ...
        # self.tokens_map = nn.Embedding(token_num, 1) 

        # Stem
        self.stem = nn.Sequential(
            nn.Conv3d(in_chans, stem_chs, (1,3,3), stride=(1,2,2),padding=(0,1,1), bias=False),
            nn.BatchNorm3d(stem_chs),
            nn.ReLU6(inplace=True)
        )
        self.SE = SELayer(channel=stem_chs)
        ...
        # r3d_model_18 = resnet.generate_model(
        #     model_depth = 18
        # )
        # self.r3d_model_18_pre = load_pretrained_model(
        #     r3d_model_18,
        #     pretrain_path = '/wangxiao/ycg/MF3D/pre_r3d/r3d18_K_200ep.pth',
        #     model_name = 'resnet',
...
def mobile_former_508m(pretrained=False, **kwargs):
    ...
    dna_blocks = [ 
        # b, e1,  c, n, s, e2
        ['DnaBlock3', 2,  24, 1, 1, 0], #1 112x112 (1) [32, 24, 8, 112, 112]
        ['DnaBlock3', 6,  40, 1, 2, 4], #2 56x56 (2) [32, 40, 4, 56, 56]
        ['DnaBlock',  3,  40, 1, 1, 3], #3
        ['DnaBlock3', 6,  72, 1, 2, 4], #4 28x28 (2) [32, 72, 2, 28, 28]
        ['DnaBlock',  3,  72, 1, 1, 3], #5 
        ['DnaBlock3', 6, 128, 1, 2, 4], #6 14x14 (4) #torch.Size([32, 128, 1, 14, 14])
        ['DnaBlock',  4, 128, 1, 1, 4], #7
        ['DnaBlock',  6, 176, 1, 1, 4], #8
        ['DnaBlock',  6, 176, 1, 1, 4], #9
        ['DnaBlock3', 6, 240, 1, 2, 4], #10 7x7 (3) [32, 240, 1, 7, 7]
